...
import Swal from 'sweetalert2';
  ...
  const fetchElders = async(page) =>{
    try {
      const res = await fetch(`http://localhost:8080/api/importRoute/?page=${page}&search=${searchQuery}`, {
        headers: {
  ...
  if (file) {
    ...
    reader.onload = (e) => {
      try {
        ...
        fetch('http://localhost:8080/api/importRoute/upload', {
          method: 'POST',
          body: formData,
          headers: {
            'Authorization': `Bearer ${user.token}`,
    ...
    const deleteElder = async (ImportDataID) => {
      try {
        const response = await fetch(`http://localhost:8080/api/importRoute/${ImportDataID}`, {
          method: 'DELETE',
          headers: {
            'Authorization': `Bearer ${user.token}`,
          }
