...
function IDverification()  {
    ...
    useEffect(() => {
      fetchUsers(currentPage)
       ...
       const fetchUsers = async(page,userID) =>{
         const res = await fetch(`http://localhost:8080/api/usersRoute/?page=${page}&search=${searchQuery}`, {
           headers: {
  ...
  //user getting moved to Verified List
    const moveElder =  async (userID) => {
      ...
      try{
    const response = await fetch(`http://localhost:8080/api/elderRoute/move-item/${userID}`, {
        method: 'POST',
        headers: {
          'Authorization': `Bearer ${user.token}`,
        }
  ...
  //sms APi actions
    ...
    const sendSMS = async(userID,messageText) =>{
      try{
        const res = await fetch(`http://localhost:8080/api/smsRoute/sendSmS/${userID}`, {
          method: 'POST',
          headers: {
            'Authorization': `Bearer ${user.token}`,
    ...
    const sendSMSbulk = async(selectedRows,messageText) =>{
      try{
        const res = await fetch(`http://localhost:8080/api/smsRoute/sendToMany/${selectedRows}`, {
          method: 'POST',
          headers: {
            'Authorization': `Bearer ${user.token}`,
            'Content-Type': 'application/json',
